export async function checkSingleField(component, field, data) {
    if (component  && component.validations && component.className && component.validationSource) {
        var petition = {
            response: true,
            method: "post",
            body: {
        ...
        var response = await submitRequest($larafetch("/api/v1/form/prevalidation", petition));
        if (!response.data) {
        ...
        console.error('Variables necesarias no existen en el componente checkSingleField');
    }
}
